...
import reactive.AtomicRef
import reactive.web.Page
import reactive.web.PagesCache
import reactive.web.Transport
import reactive.web.TransportType
import reactive.web.lift

import java.io.{ OutputStream, OutputStreamWriter, Writer }
import reactive.web.lift.SseTransportType
 ...
 */
object MySseTransportType extends PagesCache with Loggable {
  /**
  ...
  def init(): Unit = {
    LiftRules.dispatch append {
      case req @ Req("__reactive-web-sse" :: PageById(page) :: Nil, "", GetRequest) =>
        S.respondAsync {
          page.transportTypes.collectFirst { case spc: MySseTransportType => spc } map { spc =>
 ...
 */
class MySseTransportType(page: Page) extends TransportType with Loggable {
  /**
  ...
  object sseTransport extends Transport {
    ...
    private[reactivehelpers] def write(writer: Writer, lastReceived: Long = 0): Unit = {
  ...
  MySseTransportType.addPage(page)
}
